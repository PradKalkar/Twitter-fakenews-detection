...
def process_dataset(model):
    ...
    for idx in range(len(tweet_df)):
        try:
            ...
            tweet = str(tweet_df.loc[idx, "Tweet"]) # current tweet
            tweetCreatedAt = tweet_df.loc[idx, "TweetCreatedAt"]
            top_k_premises = str(tweet_df.loc[idx, "TopKHashAndMentionTweets"]) # top k premises
            label = str(tweet_df.loc[idx, "Label"])
            ...
            prediction = entailment_without_finetuning(model, tweet, relevant_sentences_premise, threshold=0.5)
